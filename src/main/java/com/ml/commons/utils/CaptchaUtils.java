...
public class CaptchaUtils {
    ...
    private static String drawGraphic(BufferedImage image) {
        ...
        g.setStroke(bs);
        //画出曲线
        QuadCurve2D.Double curve = new QuadCurve2D.Double(0d, random
                .nextInt(HEIGHT - 8) + 4, WIDTH / 2, HEIGHT / 2, WIDTH, random.nextInt(HEIGHT - 8) + 4);
        g.draw(curve);
        // 销毁图像
        g.dispose();
        return sRand;
     ...
     * @return @return 验证通过返回 true, 否则返回 false
     */
